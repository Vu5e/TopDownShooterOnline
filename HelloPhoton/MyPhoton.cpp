...
void MyPhoton::sendMyData(Spaceship mySpaceship, Bullet* myBulletsArray[100], int myBulletsCount)
{
	float data[200];
	data[0] = mySpaceship.getPos().mVal[0];
	data[1] = mySpaceship.getPos().mVal[1];
	data[2] = mySpaceship.getVelocity().mVal[0];
	data[3] = mySpaceship.getVelocity().mVal[1];
	data[4] = mySpaceship.getAcceleration().mVal[0];
	data[5] = mySpaceship.getAcceleration().mVal[1];
	data[6] = mySpaceship.getRotation();
	data[7] = myBulletsCount;

	int size = 8 + (myBulletsCount*4);
	if (size > 200)
	{
		std::cout << "Cannot send more than " << size << " varibles, please increase the array size" << std::endl;
		return;
	}

	int count = 8;

	for (int i = 0; i < myBulletsCount; i++)
	{
		//insert x position
		data[count] = myBulletsArray[i]->getPos().mVal[0];

		count++;

		//insert y position
		data[count] = myBulletsArray[i]->getPos().mVal[1];

		count++;
		

		//insert x velocity
		data[count] = myBulletsArray[i]->getVelocity().mVal[0];

		count++;

		//insert y velocity
		data[count] = myBulletsArray[i]->getVelocity().mVal[1];
...
void MyPhoton::sendMyShot(int playerID, float command, Vector target)
{
	float data[4];
	data[0] = playerID;
	data[1] = command;
	data[2] = target.mVal[0];
	data[3] = target.mVal[1];

	mLoadBalancingClient.opRaiseEvent(true, data, 4, 2);
}

void MyPhoton::sendDamageNotify(int playerID, int damage)
{
	int data[2];
	data[0] = playerID;
	data[1] = damage;

	mLoadBalancingClient.opRaiseEvent(true, data, 2, 3);
}
//New addition
void MyPhoton::sendMyMissile(int playerID, float command, Vector target)
{
	float data[4];
	data[0] = playerID;
	data[1] = command;
	...
	mLoadBalancingClient.opRaiseEvent(true, data, 4, 4);
}
...
void MyPhoton::customEventAction(int playerNr, nByte eventCode, const ExitGames::Common::Object& eventContent)
{
	...
	else if (eventCode == 2)
	{
		...
		if (data)
		{
			Application::getInstance().OnReceivedOpponentCommand(data);
		}
		else
			std::cout << "invalid data" << std::endl;
	}
	else if (eventCode == 3)
	{
		...
		if (data)
		{
			Application::getInstance().OnReceivedDamageNotify(data);
		}
		else
			std::cout << "invalid data" << std::endl;
	}
	else if (eventCode == 4) //New addition
	{
		float* data = ExitGames::Common::ValueObject<float*>(eventContent).getDataCopy();
		if (data)
		{
			Application::getInstance().OnReceivedOpponentMissile(data);
		}
		else
			std::cout << "invalid data" << std::endl;
	}
...
{
	Application::getInstance().PlayerID = 1;
...
void MyPhoton::joinRandomRoomReturn(int localPlayerNr, const ExitGames::Common::Hashtable& /*gameProperties*/, const ExitGames::Common::Hashtable& /*playerProperties*/, int errorCode, const ExitGames::Common::JString& errorString)
{
	...
	if (localPlayerNr >= 2)
	{
		Application::getInstance().PlayerID = 2;
	}
}
